...
class MainApplication:
    ...
    def _show_device_window(self, device_id, device_name, channels):
        ...
        def do_win_export():
            ...
            for i, w in enumerate([18, 24, 25, 10, 15, 10, 10, 15, 10, 10, 12, 18, 10, 10, 12], 1):
        ...
        try:
            ...
            for i, row_data in enumerate(all_rows, 1):
                for j, v in enumerate(row_data, 1):
                    ws.cell(row=i + 1, column=j, value=v)
            col_widths = [18, 24, 25, 10, 15, 10, 10, 15, 10, 10, 12, 18, 10, 10, 12]
    ...
    def download_import_template(self):
        ...
        try:
            ...
            columns = [
                ...
                ("治理后名称", 12, EDITABLE_FILL, GREEN_FONT, "✎ 可修改"),
            ]
            ...
            usage = [
                ...
                ("", False),
                ("  绿色表头（✎）= 可修改字段：", False),
                ("      通道名称、治理后名称、区域编码、经度、纬度、厂家、设备型号、设备归属、地址、密码", False),
    ...
    def export_excel(self):
        ...
        try:
            ...
            for i, ch in enumerate(self.all_channels, 1):
                ...
                for j, v in enumerate(row, 1):
                    ws.cell(row=i+1, column=j, value=v)
            for i, w in enumerate([6, 22, 25, 10, 15, 10, 10, 15, 10, 10, 12, 18, 10, 10, 12], 1):
                ws.column_dimensions[ws.cell(1, i).column_letter].width = w
            wb.save(file)
